...
bool e12aio_httpd_auth_valid(httpd_req_t *req) {
  ...
  if (httpd_req_get_hdr_value_str(req, "Authorization", l_hdr_buff,
                                  l_hdr_len + 1) == ESP_OK) {
    char l_decoded[50];
    ...
    if (mbedtls_base64_decode((unsigned char *)&l_decoded, 50, &l_decodedSize,
                              (unsigned char *)l_partAuth64,
                              strlen(l_partAuth64)) == 0) {
      l_decoded[l_decodedSize] = 0;
      char *l_auth_username = strtok(l_decoded, ":");
      char *l_auth_password = strtok(NULL, ":");
      ESP_LOGD(TAG, "Auth Decoded - User: [%s] - Password: [%s]",
               l_auth_username, l_auth_password);
      ...
      ESP_LOGD(TAG, "Auth Config - User: [%s] - Password: [%s]",
               l_config_username, l_config_password);
      if (strncasecmp(l_auth_username, l_config_username,
                      strlen(l_config_username)) == 0 &&
          strncasecmp(l_auth_password, l_config_password,
                      strlen(l_config_password)) == 0)
        return true;
    }
  }
  return false;
...
/**
 * @brief This function handle all basic authentication needs. Showing form if
 * it is necessary.
 * @param req http request
 * @return
...
esp_err_t e12aio_httpd_handler_spiffs(httpd_req_t *req) {
  ...
  size_t l_size = 0;
  char l_filename[E12AIO_MAX_FILENAME * 2];
  ...
  if (l_size > 0) {
    ...
    httpd_query_key_value(l_buffer, "file", l_partialFileName,
                          E12AIO_MAX_FILENAME);
    snprintf(l_filename, E12AIO_MAX_FILENAME * 2, "%s/%s",
             e12aio_spiffs_get_basepath(), l_partialFileName);
  } else
  ...
  do {
    l_size = e12aio_spiffs_read(l_filename, (char *)&l_buffer,
 ...
 * Sample: { "status": "ok", "board": { "model": "E12-AIO3", "hostname":
 * "e12aio_CC8C20", "build": "2.0.0", "uptime": "00:11:52" }, "wifi": { "mode":
 * "STA", "status": "connected", "ip": "10.15.197.199", "gateway": "10.15.20.1",
 * "netmask": "255.255.0.0" }, "mqtt": { "status": "connected" } }
 */
...
esp_err_t e12aio_httpd_handler_status(httpd_req_t *req) {
  ...
  cJSON *l_mqtt_status = cJSON_CreateString(
      e12aio_mqtt_is_connected() ? "connected" : "disconnected");
  ...
  char l_buffer[CONFIG_JSON_BUFFER_SIZE];
  cJSON_PrintPreallocated(l_json, (char *)&l_buffer, CONFIG_JSON_BUFFER_SIZE,
                          false);
  cJSON_Delete(l_json);
  httpd_resp_send(req, l_buffer, strlen(l_buffer));
  return ESP_OK;
...
esp_err_t e12aio_httpd_handler_api(httpd_req_t *req) {
  ...
  httpd_query_key_value(l_buffer, "token", (char *)&l_token,
...
esp_err_t e12aio_httpd_handler_action(httpd_req_t *req) {
  ...
  if (strncmp(l_name, "restart", 7) == 0) {
    ...
    // TODO: Convert from GET to POST (permitting a use of larger URLs like
    // GitHub, OneDrive, etc.).
