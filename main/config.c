...
void e12aio_config_init() {
  ...
  if (!s_initialized) {
    ...
    xEventGroupWaitBits(g_eventGroup, E12AIO_CONFIG_LOADED, pdFALSE, pdTRUE,
                        portMAX_DELAY);
  }
}
...
char *e12aio_config_get_name() {
  ...
  snprintf(s_esp_name, sizeof(s_esp_name), "e12aio_%02X%02X%02X", l_mac[3],
           l_mac[4], l_mac[5]);
...
void e12aio_config_prepare_configs() {
  ...
  if (e12aio_spiffs_read(g_config_file, (char *)&l_buffer,
                         CONFIG_JSON_BUFFER_SIZE) > 0) {
...
void e12aio_config_lazy_save_task(void *args) {
  ...
  xEventGroupWaitBits(g_eventGroup, E12AIO_CONFIG_DELAYED_SAVE_RIGHTNOW, pdTRUE,
                      pdTRUE, time / portTICK_PERIOD_MS);
...
/**
 * Save all your configurations in a lazy mode (avoid to many simultaenous
 * write)
 */
...
/**
 * Save all your configurations in a lazy mode (avoid to many simultaenous
 * write) after some delay milliseconds
 */
void e12aio_config_lazy_save_after(const unsigned long delay) {
  if (!e12aio_config_lazy_started()) {
    ...
    xEventGroupClearBits(g_eventGroup, E12AIO_CONFIG_DELAYED_SAVE_RIGHTNOW);
    xTaskCreate(e12aio_config_lazy_save_task, "config_lazy_save", 4096,
                (void *)delay, 1, NULL);
  } else if (delay == 0) {
    // make save right now!
...
void e12aio_config_load_from_buffer(const char *buffer) {
  ...
  strncpy(g_config.wifi.sta_ssid,
          l_wifi_sta_ssid != NULL ? l_wifi_sta_ssid->valuestring
                                  : CONFIG_WIFI_SSID,
          E12AIO_WIFI_SSID_SIZE);
  strncpy(g_config.wifi.sta_password,
          l_wifi_sta_password != NULL ? l_wifi_sta_password->valuestring
                                      : CONFIG_WIFI_PASSWORD,
          E12AIO_WIFI_PASSWORD_SIZE);
  strncpy(g_config.wifi.ap_password,
          l_wifi_ap_password != NULL
              ? l_wifi_ap_password->valuestring
              : (strlen(CONFIG_WIFI_AP_PASSWORD) < 8 ? "password"
                                                     : CONFIG_WIFI_AP_PASSWORD),
          E12AIO_WIFI_PASSWORD_SIZE);
  g_config.wifi.dhcp = (l_wifi_dhcp != NULL) ? cJSON_IsTrue(l_wifi_dhcp) : true;
  strncpy(g_config.wifi.ip,
          l_wifi_ip != NULL ? l_wifi_ip->valuestring : "192.168.1.2",
          E12AIO_IPV4_SIZE);
  strncpy(g_config.wifi.netmask,
          l_wifi_netmask != NULL ? l_wifi_netmask->valuestring
                                 : "255.255.255.0",
          E12AIO_IPV4_SIZE);
  strncpy(g_config.wifi.gateway,
          l_wifi_gateway != NULL ? l_wifi_gateway->valuestring : "192.168.1.1",
          E12AIO_IPV4_SIZE);
  strncpy(g_config.wifi.dns1,
          l_wifi_dns1 != NULL ? l_wifi_dns1->valuestring : "8.8.8.8",
          E12AIO_IPV4_SIZE);
  strncpy(g_config.wifi.dns2,
          l_wifi_dns2 != NULL ? l_wifi_dns2->valuestring : "8.8.4.4",
          E12AIO_IPV4_SIZE);
  ...
  g_config.mqtt.enable =
      (l_mqtt_enable != NULL) ? cJSON_IsTrue(l_mqtt_enable) : true;
  strncpy(g_config.mqtt.url,
          l_mqtt_url != NULL ? l_mqtt_url->valuestring : CONFIG_MQTT_URL,
          E12AIO_MQTT_URL_SIZE);
  strncpy(g_config.mqtt.topic,
          l_mqtt_topic != NULL ? l_mqtt_topic->valuestring
                               : CONFIG_MQTT_TOPIC_BASE,
          CONFIG_MQTT_TOPIC_SIZE);
  ...
  strncpy(g_config.httpd.username,
          l_httpd_username != NULL ? l_httpd_username->valuestring
                                   : CONFIG_WEB_AUTH_USERNAME,
          CONFIG_WEB_AUTH_MAX_SIZE);
  strncpy(g_config.httpd.password,
          l_httpd_password != NULL ? l_httpd_password->valuestring
                                   : CONFIG_WEB_AUTH_PASSWORD,
          CONFIG_WEB_AUTH_MAX_SIZE);
  strncpy(g_config.httpd.token,
          l_httpd_token != NULL ? l_httpd_token->valuestring : CONFIG_WEB_TOKEN,
          CONFIG_WEB_AUTH_MAX_SIZE);
  g_config.httpd.api_enable =
      (l_httpd_api_enable != NULL) ? cJSON_IsTrue(l_httpd_api_enable) : true;
  ...
  g_config.ota.state =
      (l_ota_state != NULL ? l_ota_state->valueint : E12AIO_OTA_OK);
  strncpy(g_config.ota.version,
          l_ota_version != NULL ? l_ota_version->valuestring : E12AIO_VERSION,
          E12AIO_OTA_VERSION_SIZE);
...
void e12aio_config_save() {
  char l_buffer[CONFIG_JSON_BUFFER_SIZE];
  size_t l_len = e12aio_config_save_buffer_adv(g_config, (char *)l_buffer,
...
size_t e12aio_config_save_buffer_adv(e12aio_config_t data, char *buffer,
                                     size_t sz) {
...
void e12aio_config_dump() {
  ...
  ESP_LOGD(TAG, "MQTT - Enable: %s",
           g_config.mqtt.enable ? "Enable" : "Disable");
  ...
  ESP_LOGD(TAG, "HTTPD - API: %s",
           g_config.httpd.api_enable ? "Enable" : "Disable");
...
e12aio_config_t *e12aio_config_get() { return &g_config; }
